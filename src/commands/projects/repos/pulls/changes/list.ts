...
export default class List extends BaseCommand {
    ...
    static examples = [
        `$ stash projects:repost:pulls:changes:list -a MyStashAlias --project "ProjectKey" --slug "MyRepoSlug" --pull 1234 --all --csv`,
        `$ stash projects:repost:pulls:changes:list -a MyStashAlias --project "ProjectKey" --slug "MyRepoSlug" --pull 1234 -l 100 -s 50 --json`,
        `$ stash projects:repost:pulls:changes:list -a MyStashAlias --project "ProjectKey" --slug "MyRepoSlug" --pull 1234 --from 5678 --limit 30`,
    ];
    static flags = {
        ...BaseCommand.flags,
        csv: BuildFlags.csv,
    ...
    async run(): Promise<StashCLIResponse<Page<RepoChangesOutput>>> {
        ...
        try {
            ...
            if (this.flags.all) {
                let tmp = await connector.projects.repos(this.flags.project).pullRequests(this.flags.slug).changes(this.flags.pull).list(this.flags['with-comments'], this.allPageOptions);
                result.values.push(...tmp.values);
                result.isLastPage = true;
                result.start = tmp.start;
                while (!tmp.isLastPage) {
                    tmp = await connector.projects.repos(this.flags.project).pullRequests(this.flags.slug).changes(this.flags.pull).list(this.flags['with-comments'], {
                ...
                result = await connector.projects.repos(this.flags.project).pullRequests(this.flags.slug).changes(this.flags.pull).list(this.flags['with-comments'], this.pageOptions);
            }
